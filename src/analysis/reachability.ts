...
const DEFAULT_IGNORE_ALWAYS: RegExp[] = [
  /\.d\.ts$/,
]

const DEFAULT_IGNORE_GENERATED: RegExp[] = [
  /\/generated\//,
  /\.gen\.(ts|tsx|js|jsx)$/,
  /\.generated\.(ts|tsx|js|jsx)$/,
]

const DEFAULT_ROOT_ENTRY_RELATIVE_REGEXES: RegExp[] = [
...
export function computeReachableFiles(options: {
  ...
  for (const sf of sourceFiles) {
    ...
    if (shouldIgnoreFile(abs, options.ignoreGenerated ?? true)) continue
    if (shouldIgnoreFrameworkFile(abs, projectRoot, options.entryPointConfig)) continue
  ...
  function visit(filePath: string): void {
    ...
    if (reachableFiles.has(abs)) return
    if (!fileByPath.has(abs)) return
    ...
    for (const specifier of specifiers) {
      ...
      if (shouldIgnoreFile(resolved, options.ignoreGenerated ?? true)) continue
      visit(resolved)
    }
  }
...
function shouldIgnoreFile(filePathAbs: string, ignoreGenerated: boolean): boolean {
  const normalized = filePathAbs.replace(/\\/g, '/')
  if (DEFAULT_IGNORE_PATH_PARTS.some((p) => normalized.includes(p))) return true
  if (DEFAULT_IGNORE_ALWAYS.some((r) => r.test(normalized))) return true
  if (ignoreGenerated && DEFAULT_IGNORE_GENERATED.some((r) => r.test(normalized))) return true
  return false
}

function getModuleSpecifiers(sourceFile: SourceFile): string[] {
  const out: string[] = []
