 ...
 */
export function formatGitHubActions(issues: Issue[], projectRoot: string): string {
  const lines: string[] = []
  ...
  for (const issue of issues) {
    ...
    lines.push(
      `::${level} file=${relativePath},line=${issue.line},col=${issue.column},title=${title}::${issue.message}`
    )
  }
 ...
 */
export function formatCSV(issues: Issue[], projectRoot: string): string {
  const lines: string[] = []
...
function formatIssueTitle(kind: IssueKind): string {
...
function getIssueDescription(kind: IssueKind): string {
  const descriptions: Record<IssueKind, string> = {
