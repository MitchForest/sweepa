...
export class CallGraphBuilder {
    ...
    build() {
        ...
        for (const sourceFile of sourceFiles) {
            if (this.shouldExclude(sourceFile.getFilePath()))
                continue;
     ...
     */
    findOutgoingReferences(node, fromId) {
        // Skip if source doesn't exist in graph
        if (!this.graph.hasSymbol(fromId))
            return;
        ...
        for (const identifier of identifiers) {
            try {
                ...
                if (!targetId || !targetName || !targetKind)
                    continue;
     ...
     */
    resolveImportSpecifier(importSpec) {
        if (!Node.isImportSpecifier(importSpec))
            return undefined;
        ...
        const moduleSourceFile = sourceFile.getProject()
            ...
            return (filePath.includes(moduleSpec.replace(/^[./]+/, '')) &&
     ...
     */
    findOutgoingTypeReferences(node, fromId) {
        // Skip if source doesn't exist in graph
        if (!this.graph.hasSymbol(fromId))
            return;
        const filePath = node.getSourceFile().getFilePath();
     ...
     */
    ensureModuleNode(filePath, moduleId) {
        if (this.graph.hasSymbol(moduleId))
            return;
