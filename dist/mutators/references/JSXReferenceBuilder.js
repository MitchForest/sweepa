...
export const JSXReferenceBuilder = {
    ...
    mutate(ctx) {
        ...
        for (const sourceFile of project.getSourceFiles()) {
            ...
            for (const jsx of jsxElements) {
                ...
                if (!graph.hasSymbol(containerId))
                    continue;
...
function findContainingDeclaration(node) {
    ...
    while (current) {
        ...
        if (Node.isMethodDeclaration(current)) {
            ...
            if (name && Node.isClassDeclaration(cls)) {
                ...
                if (clsName)
                    return { name: `${clsName}.${name}`, kind: 'function' };
            }
        }
...
function buildSymbolId(filePath, name) {
    return `${filePath}:${name}`;
}
function findComponentSymbol(graph, tagName, sourceFile, projectRoot) {
...
//# sourceMappingURL=JSXReferenceBuilder.js.map