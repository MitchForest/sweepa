...
export const EntryPointRetainer = {
    ...
    mutate(ctx) {
        ...
        const honoModulePatterns = entryPatterns.filter((p) => p.filePattern && p.reason.toLowerCase().startsWith('hono '));
...
function buildEntryPatterns(frameworks) {
    const patterns = [];
    for (const fw of frameworks) {
        ...
        switch (fwName) {
            case 'tanstack-start':
                ...
                { filePattern: /(^|\/)routes\/.*\.(tsx?|jsx?)$/, exportPattern: 'Route', reason: 'TanStack route' }, { filePattern: /(^|\/)routes\/.*\.(tsx?|jsx?)$/, exportPattern: 'loader', reason: 'TanStack loader' }, { filePattern: /(^|\/)routes\/.*\.(tsx?|jsx?)$/, exportPattern: 'action', reason: 'TanStack action' }, { filePattern: /(^|\/)routes\/.*\.(tsx?|jsx?)$/, exportPattern: /^[A-Z]/, reason: 'TanStack route component' }, 
                // Root router
                { filePattern: /(^|\/)router\.(tsx?|jsx?)$/, reason: 'TanStack router' }, { filePattern: /routeTree\.gen\.(tsx?|jsx?)$/, reason: 'TanStack route tree' });
                break;
            case 'next':
            case 'next.js':
                patterns.push(
                // Pages router
                { filePattern: /(^|\/)pages\/.*\.(tsx?|jsx?)$/, exportPattern: 'default', reason: 'Next.js page' }, { filePattern: /(^|\/)pages\/.*\.(tsx?|jsx?)$/, exportPattern: 'getServerSideProps', reason: 'Next.js SSR' }, { filePattern: /(^|\/)pages\/.*\.(tsx?|jsx?)$/, exportPattern: 'getStaticProps', reason: 'Next.js SSG' }, { filePattern: /(^|\/)pages\/.*\.(tsx?|jsx?)$/, exportPattern: 'getStaticPaths', reason: 'Next.js paths' }, 
                // App router
                { filePattern: /(^|\/)app\/.*\/page\.(tsx?|jsx?)$/, exportPattern: 'default', reason: 'Next.js app page' }, { filePattern: /(^|\/)app\/.*\/layout\.(tsx?|jsx?)$/, exportPattern: 'default', reason: 'Next.js layout' }, { filePattern: /(^|\/)app\/.*\/loading\.(tsx?|jsx?)$/, exportPattern: 'default', reason: 'Next.js loading' }, { filePattern: /(^|\/)app\/.*\/error\.(tsx?|jsx?)$/, exportPattern: 'default', reason: 'Next.js error' }, 
                // API routes
                { filePattern: /(^|\/)pages\/api\/.*\.(tsx?|jsx?)$/, exportPattern: 'default', reason: 'Next.js API route' }, { filePattern: /(^|\/)app\/api\/.*\/route\.(tsx?|jsx?)$/, reason: 'Next.js route handler' });
                break;
            case 'hono':
                patterns.push(
                // Main app entry - only root level index
                { filePattern: /^(src\/)?index\.(tsx?|jsx?)$/, reason: 'Hono app entry' }, { filePattern: /^(src\/)?app\.(tsx?|jsx?)$/, reason: 'Hono app' }, { filePattern: /^(src\/)?server\.(tsx?|jsx?)$/, reason: 'Hono server' }, 
            ...
            case 'jest':
                ...
                { filePattern: /\.(test|spec)\.(tsx?|jsx?)$/, reason: `${fw.name} test file` }, { filePattern: /(^|\/)__tests__\/.*\.(tsx?|jsx?)$/, reason: `${fw.name} test file` });
                break;
            case 'drizzle':
                patterns.push(
                // Schema files
                { filePattern: /(^|\/)schema\.(tsx?|jsx?)$/, reason: 'Drizzle schema' }, { filePattern: /(^|\/)db\/.*\.(tsx?|jsx?)$/, reason: 'Drizzle database' });
                break;
            default:
...
function matchesEntryPattern(filePath, exportName, pattern) {
    // Check file pattern
...
//# sourceMappingURL=EntryPointRetainer.js.map