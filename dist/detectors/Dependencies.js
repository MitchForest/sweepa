...
import { findNearestPackageJson, readPackageJson, getAllListedDependencies, isNodeBuiltin, isRelativeOrAbsolute, getPackageNameFromSpecifier, } from '../deps/index.js';
...
export function detectDependencyIssues(options) {
    ...
    for (const sf of options.project.getSourceFiles()) {
        ...
        if (!options.reachableFiles.has(filePath) && !isDevDependencyEntryFile(projectRoot, filePath))
            continue;
    ...
    if (listed.has('react-dom') && listed.has('@types/react-dom')) {
        const existing = usages.get('@types/react-dom') ?? {
            usedInProd: false,
            usedInDev: true,
            byFiles: new Set(['<react-dom-types>']),
            unresolvedSpecifiers: new Set(),
        };
        existing.usedInDev = true;
...
function classifyFileUsageKind(projectRoot, filePathAbs) {
    ...
    if (/(^|\/)bin\//.test(rel))
        return 'dev';
    if (/(^|\/)__tests__\//.test(rel))
        return 'dev';
    if (/(^|\/)tests?\//.test(rel))
        return 'dev';
    if (/\.(test|spec)\.[jt]sx?$/.test(rel))
        return 'dev';
    return 'prod';
}
function isDevDependencyEntryFile(projectRoot, filePathAbs) {
    const rel = path.relative(projectRoot, filePathAbs).replace(/\\/g, '/');
    const base = path.basename(filePathAbs);
    // Common config entrypoints that influence dependency usage.
    if (base === 'eslint.config.js' || base === 'eslint.config.mjs' || base === 'eslint.config.cjs')
        return true;
    if (base.startsWith('vite.config.'))
        return true;
    if (base.startsWith('vitest.config.'))
        return true;
    if (base.startsWith('drizzle.config.'))
        return true;
    if (base.endsWith('.config.ts') || base.endsWith('.config.js') || base.endsWith('.config.mjs') || base.endsWith('.config.cjs')) {
        return true;
    }
...
function getAllSpecifiers(sourceFile) {
    const out = [];
...
function looksLikeCssImport(specifier) {
    return specifier.includes('.css');
}
function resolveRelativeAsset(containingFileAbs, specifier) {
    try {
        ...
        if (fs.existsSync(full) && full.endsWith('.css'))
            return full;
        return undefined;
    }
    catch {
...
function extractCssPackageImports(cssFileAbs) {
    const out = new Set();
    try {
...
function markUsage(usages, pkgName, usageKind, filePath, label) {
    const existing = usages.get(pkgName) ?? {
        usedInProd: false,
        usedInDev: false,
        byFiles: new Set(),
...
function extractToolsFromScript(script) {
    const out = new Set();
...
function mapScriptToolToPackage(tool, listed) {
    ...
    if (mapped && listed.has(mapped))
        return mapped;
    return undefined;
}
function scriptsMentionBun(pkg) {
...
function applyConfigFileUsage(options) {
    ...
    for (const name of candidates) {
        ...
        for (const spec of imports) {
            ...
            if (!options.listed.has(pkgName))
                continue;
            markUsage(options.usages, pkgName, 'dev', options.packageJsonPath, `<config:${name}>`);
        }
    }
}
function extractJsImportsFromTextFile(fileAbs) {
    const out = new Set();
    try {
