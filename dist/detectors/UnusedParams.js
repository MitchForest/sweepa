 ...
 */
export function detectUnusedParams(project, options = {}) {
    const issues = [];
    const { ignoreUnderscoreParams = true, ignoreInterfaceParams = true, } = options;
 ...
 */
function isParameterUsed(fn, param) {
    const body = fn.getBody();
    if (!body)
        return true; // No body = can't determine, assume used
    const nameNode = param.getNameNode();
 ...
 */
function findReferencesInNode(node, name) {
    const refs = [];
 ...
 */
function isRequiredByInterface(fn, param) {
 ...
 */
function findReferencesInBody(body, name) {
    let count = 0;
 ...
 */
function determineConfidence(fn, _param) {
...
//# sourceMappingURL=UnusedParams.js.map