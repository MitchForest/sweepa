 ...
 */
export function detectUnusedMethods(project, options = {}) {
    const issues = [];
    const { ignoreDecorators = [], ignoreLifecycleMethods = true, ignoreUnderscoreMethods = true, } = options;
 ...
 */
function findMethodReferences(method, _project) {
    const references = [];
    try {
 ...
 */
function hasIgnoredDecorator(method, ignoreDecorators) {
    if (ignoreDecorators.length === 0)
        return false;
 ...
 */
function isOverrideMethod(method) {
 ...
 */
function determineConfidence(method, classExported) {
...
//# sourceMappingURL=UnusedMethods.js.map