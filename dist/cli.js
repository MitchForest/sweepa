...
import { createBaseline, filterNewIssues, validateBaseline, } from './baseline/index.js';
import { shouldIgnoreIssue, } from './config/index.js';
import { formatGitHubActions, formatGitHubMarkdown, formatSARIF, formatCSV, } from './output/index.js';
    ...
    try {
        ...
        if (runUnusedFiles) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking unused files...'));
        ...
        if (runUnusedTypes) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking unused types...'));
        ...
        if (runUnusedExports) {
            ...
            if (options.reachability) {
                if (!quiet)
                    console.log(chalk.gray('  Running mutator pipeline (reachability analysis)...'));
        ...
        if (runUnusedParams) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking unused parameters...'));
            const issues = detectUnusedParams(project);
            allIssues.push(...filterIgnoredIssues(issues, ignores));
        }
        if (runUnusedMethods) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking unused methods...'));
            const issues = detectUnusedMethods(project);
            allIssues.push(...filterIgnoredIssues(issues, ignores));
        }
        if (runUnusedImports) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking unused imports...'));
            const issues = detectUnusedImports(project);
            allIssues.push(...filterIgnoredIssues(issues, ignores));
        }
        if (runUnusedEnums) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking unused enum cases...'));
            const issues = detectUnusedEnumCases(project);
            allIssues.push(...filterIgnoredIssues(issues, ignores));
        }
        if (runRedundantExports) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking redundant exports...'));
            const issues = detectRedundantExports(project, projectRoot);
            allIssues.push(...filterIgnoredIssues(issues, ignores));
        }
        if (runAssignOnly) {
            if (!quiet && format === 'console')
                console.log(chalk.gray('  Checking assign-only properties...'));
...
function filterIgnoredIssues(issues, ignores) {
...
function outputResults(issues, format, projectRoot, graphStats, baselineInfo, quiet) {
        ...
        case 'json':
            console.log(JSON.stringify({
                stats: {
                    ...
                    issuesByKind: Object.fromEntries([...issuesByKind.entries()].map(([k, v]) => [k, v.length])),
                },
                issues: issues.map(i => ({
                    kind: i.kind,
        ...
        default:
            ...
            for (const [kind, kindIssues] of issuesByKind.entries()) {
                ...
                for (const issue of toShow) {
                    ...
                    console.log(`  ${chalk.red('✗')} ${chalk.bold(issue.name)}${parentStr} ` +
                        confBadge +
